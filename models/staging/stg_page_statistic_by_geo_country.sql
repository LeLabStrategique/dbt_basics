...
deduplication AS (
    SELECT
        ...
        CONCAT(
            s.day_format, 
            '_', 
            COALESCE(s.dim_geo_country_name_enriched, CAST(s.geo_id AS STRING)) 
        ) AS row_id,
...
SELECT
    ...
    COALESCE(d.dim_geo_country_name_enriched, CAST(d.geo_id AS STRING)) AS dim_geo_country,
    
    -- 5. Dimension_ID
    d.geo_id AS dim_geo_country_id,
