...
WITH source_data AS (
    ...
    SELECT
        -- Exclure les colonnes source non désirées ou renommées
        f.* EXCEPT(_fivetran_id, day, organization_entity), 
        
        -- Clé de temps (UTILISÉE POUR LA LOGIQUE, mais non sélectionnée au final)
...
SELECT
    ...
    -- 4. Toutes les autres colonnes (métriques, vues, clics)
    -- EXCEPT est mis à jour pour exclure Stat_Day_Time
...
QUALIFY ROW_NUMBER() OVER (
    -- La déduplication doit toujours se baser sur le temps de synchronisation
