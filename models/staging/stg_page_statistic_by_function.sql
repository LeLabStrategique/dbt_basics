...
deduplication AS (
    SELECT
        s.*, -- Sélectionne toutes les colonnes de source_data (y compris les métriques)
...
SELECT
    ...
    COALESCE(d.dim_function_name_enriched, CAST(d.function_id AS STRING)) AS dim_function,
    
    -- 5. Dimension_ID
    d.function_id AS dim_function_id,
